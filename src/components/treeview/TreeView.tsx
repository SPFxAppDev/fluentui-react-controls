...
export class TreeView extends React.Component<ITreeViewProps, ITreeViewState> {
    ...
    public render(): React.ReactElement<ITreeViewProps> {
        return (
            <div className={css('treeview-container', this.props.className)}>
    ...
    private renderTreeList(items: ITreeItem[], level: number, ariaLabel: string): JSX.Element {
        const classNames: string[] = ['treeview-list'];
    ...
    private renderTreeItem(item: ITreeItem, level: number): JSX.Element {
        ...
        const classNames: string[] = ['treeview-list-item'];
        classNames.push(`level-${level}`);
        ...
        if (isSelected) {
            classNames.push('treeview-list-item--selected');
        ...
        if (isExpanded) {
            classNames.push('treeview-list-item--expanded');
        ...
        return (
            <li role='none' className={classNames.join(' ')} key={item.key}>
                <div
                    className={'treeview-list-item--container'}
                    role='treeitem'
                    {...ariaProperties}
                    aria-level={level}
    ...
    private onKeyDownOrUpPushed(target: HTMLElement, key: string): void {
        const allItems = document.querySelectorAll(`ul.${'treeview-list'} [role='treeitem']`);
